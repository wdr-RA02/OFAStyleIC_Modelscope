...
### 2023-04-05
结果还得采用meshed memory提供的方法, 不过辅佐降低pretrain的学习率终于也可以达到CIDEr 19的目标了

而且优化终于正了, 虽然只有+0.3. 但是阿姆斯特朗那句话怎么说来着? 一小步和一大步

今天最高兴了.jpg

pretrain的参数: 3,0.1,7.50e-05,2.25e-06,0.008,16*2
cider的参数: 2,0.06,1.00e-05,5.00e-07,0.001,8*3
(取了训练集的后50k份做CIDErD优化)

感觉scst这个东西相当的玄学, 跟step有很大的关系(即数据集大小, 卡数, batchsize都会造成很大的影响)

TODO:
1. 闲着没事再扫几组pretrain的参数(warm_up双向, lr调低, lr_end调高, 解锁resnet)
2. cider也是同样(反正有备份不慌) (lr调低, lr_end双向, 解锁resnet)
为什么说lrend有待验证呢? 因为我试过把epoch往上调, 结果还是往上走的, 所以我觉得现有的这一组还没有走到头. 但是我觉得还是可以先记录一下吧